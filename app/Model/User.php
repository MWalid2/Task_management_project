...
function update_user($conn, $data){
	$sql = "UPDATE users SET full_name=?,username=?, password=?, role=?, email=? WHERE id=? AND role=?";
	$stmt = $conn->prepare($sql);
	$stmt->execute($data);
}
...
function update_profile($conn, $data){
	$sql = "UPDATE users SET full_name=?,password=? ,  email =?WHERE id=? ";
	$stmt = $conn->prepare($sql);
	$stmt->execute($data);
}

function count_users($conn){
	$sql = "SELECT id FROM users WHERE role='employé'";
	$stmt = $conn->prepare($sql);
	$stmt->execute([]);
